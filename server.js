...
async function sendOtpEmail(toEmail, otp, type = 'reset') {
  ...
  const note = type === 'force'
    ? 'Someone is trying to force login to your account from another device. If this was you, enter the code. If not, ignore this and change your password immediately.'
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#040810;color:#d0e8ff;padding:32px;border-radius:12px;border:1px solid #1a2d4a;">
        ...
        <p style="color:#4a7090;font-size:12px;margin-bottom:24px;">${heading}</p>
        ...
        <p style="color:#4a7090;font-size:12px;">${note}</p>
      </div>
    `,
  });
